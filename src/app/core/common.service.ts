...
export class CommonService {
  ...
  filterData(filter:any,type:any) {
    if(type === "HDD") {
      this.list = this.temp.filter((row: any) => {
        if (row.HDD) {
          return (
            !filter ||
            row.HDD.toLowerCase().includes(filter.toLowerCase())
          );
        }
      });
    }
    ...
    this.isFilter.next(true);
  }

  ramFilter(filter:any) {
    this.list = this.temp.filter((row: any) => {
      if (row.RAM) {
        return (
          !filter ||
          row.RAM.toLowerCase().includes(filter.toLowerCase())
        );
      }
    });
    this.isFilter.next(true);
